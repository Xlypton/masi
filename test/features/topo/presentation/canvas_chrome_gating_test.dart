...
void main() {
  ...
  group('A-g (Bug 8): slice/AR entry points are gated on a photo actually '
      'being loaded', () {
    testWidgets('in the no-photo empty state, topo-slice-mode-button and '
        'topo-ar-button are both absent', (tester) async {
      final seeded = await _seedWall();
      addTearDown(seeded.db.close);
      addTearDown(seeded.container.dispose);
  ...
  group('A-h (Bug 9 regression): persisted slices survive a LATE-resolving '
      'slices DB read', () {
    testWidgets(
      ...
      (tester) async {
        ...
        await tester.runAsync(() async {
          ...
          await seeded.container.read(photoRepositoryProvider).replaceSlices(
            seeded.wallId,
            photoId,
            400,
            300,
  ...
  group('A-i (bug fix): a wall WITH slices is still full-bleed — PhotoSelector '
    ...
    testWidgets('a wall WITHOUT slices is unchanged: still full-bleed, and '
        'PhotoSelector is absent', (tester) async {
      const viewportSize = Size(400, 800);
      _setViewportSize(tester, viewportSize);
      final seeded = await _seedWall();
      addTearDown(seeded.db.close);
      ...
      await tester.runAsync(() async {
        await crud.attachPhotoToWall(
          seeded.wallId,
          XFile('/tmp/a-i-no-slices-photo.jpg'),
          400,
          300,
        );
      });
