...
class ArState {
  ...
  /// The route-derived rock bounding box for the current session (see
  /// `rock_box.dart`'s `rockBoxFromRoutes` — Ship 1 of the #68 AR overhaul:
  /// this REPLACES the earlier Vision-based foreground-segmentation quad/
  /// mask, which selected PEOPLE instead of the rock), in 0..1 fractions of
  /// the ORIENTED full reference photo, TL/TR/BR/BL via
  /// `rock_box.dart`'s `rockBoxCornersNorm`. `null` means either no session
  /// has started yet, or the wall's routes have no geometry to derive a box
  /// from — `ArAlignmentStage` treats both the same way (fall back to the
  /// full-photo rect as the `fromQuad` source quad; no highlight to paint).
  /// Set via [ArController.setRockBox]; reset to `null` on every AR-screen
  /// re-entry/wall switch (see `ar_screen.dart`'s `_resetArViewState`) so a
...
class ArController extends Notifier<ArState> {
  ...
  /// Records the route-derived rock box (see `rock_box.dart`'s
  /// `rockBoxFromRoutes`) for the current session, or clears it back to
  /// `null` (called with `null` by `ar_screen.dart`'s `_resetArViewState` on
  ...
  /// null out an already-set field, since passing `null` for [box] would
  /// just fall through to the CURRENT value. [ArState.rockBox] specifically
  /// must be resettable to `null` on demand (a wall switch, or a wall whose
  /// routes have no geometry to derive a box from), so it needs a real
  /// reset path rather than that shared (and, for every other field,
  /// harmless) limitation.
  void setRockBox(Rect? box) {
    state = ArState(
      mode: state.mode,
      latest: state.latest,
      active: state.active,
...
/// overlay paints the route-derived rock box ([ArState.rockBox]) as a
/// tinted outline over the tracked wall, so the user can see the (coarse)
/// area AR considers "the rock" the routes are drawn on. Defaults to off.
///
