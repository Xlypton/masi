...
- (BOOL)loadReferenceCGImage:(CGImageRef)image rockQuad:(NSArray<NSNumber *> *)rockQuad {
    ...
    // Build an ORB detection mask restricted to the rock quad (if given);
    // detection still runs on the FULL gray image otherwise -- the mask only
    // restricts WHERE keypoints are found, keypoint coordinates stay in
    // full-reference-pixel space (never cropped), matching the coordinate
    // contract in RockRegistrationEngine.swift.
    cv::Mat mask;
    if (rockQuad.count == 8) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            int px = static_cast<int>(std::lround(nx * gray.cols));
            int py = static_cast<int>(std::lround(ny * gray.rows));
            px = std::max(0, std::min(gray.cols - 1, px));
            py = std::max(0, std::min(gray.rows - 1, py));
    ...
    cv::Mat descriptors;
    orb->detectAndCompute(gray, mask.empty() ? cv::noArray() : mask, keypoints, descriptors);
...
- (nullable NSArray<NSNumber *> *)matchPixelBuffer:(CVPixelBufferRef)pixelBuffer {
    ...
    orb->detectAndCompute(gray, cv::noArray(), liveKeypoints, liveDescriptors);
