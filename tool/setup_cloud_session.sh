...
#
# The proxy's PORT ROTATES DURING A SESSION (observed: 42059 -> 40367 ->
# 32815), so it is resolved HERE, at launch, and never baked in. A stale port
# fails as ERR_PROXY_CONNECTION_FAILED on every request, which surfaces as the
# app booting signed-out — i.e. as an auth bug, in a suite that has nothing to
# do with the proxy. That cost two full E2E runs before anyone looked at the
# port. /root/.ccr/README.md is regenerated with the live port and is checked
# FIRST, because a long-running chromedriver hands this script the environment
# it was started with, which is exactly the stale one.
CCR_README=/root/.ccr/README.md
PROXY="\$(sed -n 's#.*local proxy at \\(http://[0-9.]*:[0-9]*\\).*#\\1#p' "\$CCR_README" 2>/dev/null | head -1)"
[ -n "\$PROXY" ] || PROXY="\${HTTPS_PROXY:-\${https_proxy:-}}"
