...
- **CODE** contains no catch-all around the ORIGINAL's write: the only remaining `catch (_)` is inside `_writeThumbnailBestEffort`, plus `deletePhotoBytes`' two existing ones — **3** in total. Count code only, stripping whole-line comments: `grep -v '^\s*//' lib/features/topo/data/photo_files_web.dart | grep -c 'catch (_)'` prints `3`. *(Corrected: the original assertion used a bare `grep -c 'catch (_)' …`, which prints **4** — the fourth hit is the `importPhoto` doc comment at `photo_files_web.dart:26`, which legitimately QUOTES the removed `` `catch (_) { return key; }` `` while explaining the history. The code is correct and the doc comment must NOT be reworded to satisfy a grep; the assertion was the thing that was wrong. Ninth occurrence of a bare token-grep being tripped by legitimate prose in this project — prefer a comment-stripped grep, or an assertion about behaviour, over counting raw tokens.)*
...
- There is exactly one `on PhotoWriteException catch` CLAUSE, at a LOWER line number than the `} catch (e, st) {` it precedes inside `_attachPhotoAndLoad` (Dart matches clauses top-down, so the order is the whole point). Count code only, stripping whole-line comments — `grep -v '^\s*//' lib/features/topo/presentation/topo_canvas_screen.dart | grep -n 'on PhotoWriteException catch\|} catch (e, st) {'`. *(Hardened: the bare `grep -n 'on PhotoWriteException catch' …` still returns one hit today, but `grep -rn 'on PhotoWriteException' lib` already matches prose at `photo_files_native.dart:98` and `topo_canvas_screen.dart:78` — this assertion is one prose word away from the same failure as the two corrected above, so it counts code only. Note it pins ORDER, which no test can express; the BEHAVIOUR is covered by `topo_canvas_photo_write_failure_test.dart`.)*
...
- **(Corrected — the two assertions above were the ONLY guard on this clause, and neither was a test.)** T4's container test calls `settleFailedPhotoAttach` DIRECTLY, so it proved nothing about whether the SCREEN ever calls it; the `grep -n` above passes on prose and fails on a rename. The clause was consequently deletable with the whole suite green. The real guard is now `test/features/topo/presentation/topo_canvas_photo_write_failure_test.dart`, which drives the actual screen (tap `topo-empty-state-add-photo` → pick → decode → attach → the byte write throws) and asserts the SnackBar, the cleared `selectedImageProvider`, and `isSwitchingPhoto == false`. Verified to have teeth: deleting the clause makes it fail. Reaching the clause required the smallest possible seam — `TopoCanvasScreen` gained two optional `@visibleForTesting` `photoSourcePicker`/`photoPicker` parameters defaulting to the real `showPhotoSourceSheet`/`pickPhotoFrom`, mirroring `ToposScreen`'s identical long-standing pair — because both defaults are native OS surfaces `flutter_test` cannot drive. **Consequence for `topo_canvas_photo_ops.dart`'s `settleFailedPhotoAttach` doc, which still says a widget test "cannot drive the canvas pick flow at all": that is now out of date and should be refreshed by whoever next owns that file.**
...
- There is exactly one `softDeleteWall` CALL SITE, inside `_handleNewTopo`'s `on PhotoWriteException` clause. Count code only, stripping whole-line comments: `grep -v '^\s*//' lib/features/library/presentation/topos_screen.dart | grep -c 'softDeleteWall'` prints `1`. *(Corrected: the original assertion used a bare `grep -n 'softDeleteWall' …` and expected one hit; it now returns **2** because the clause's own explanatory comment names the method while documenting why its failure is contained. Same defect class as the `catch (_)` assertion above — the code is right, the token-grep was wrong.)*
- **(F1 fix.)** That `softDeleteWall` await is wrapped in its OWN try/catch and the `photoWriteFailureSnackBar` is shown UNCONDITIONALLY. Unguarded, a cleanup failure — the same exhausted quota that refused the photo write, or `LibraryWriteLostException(ownerIdentityUnknown)` from `_guardedCascadeAllowed` — escaped the clause into the outer `debugPrint`-only catch-all, so the empty topo survived AND the user was told nothing. Locked in by the `L3/F1` group in `topos_screen_test.dart`, which fails pre-fix with "Found 0 widgets with text containing Out of storage space".
