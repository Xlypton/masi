...
### §1c verify findings — two gaps, one queued fix

§1c-A, §1c-B and §1b tasks 4–5 all **PASS** independent verification. Both headline bugs are genuinely closed on their realistic paths, with no spurious-throw regression (SQLite's `changes()` counts a matched-but-identical UPDATE as 1, so a no-op rename cannot trip the guard). Two gaps remain in the *defence-in-depth* layer.

**QUEUED FIX — the web router still ejects on a captive-portal sign-out.** A hard sign-out emits a signed-out **value**, not an `AsyncError`, so it bypasses the fail-open added in `6254fdc` and still redirects a web/PWA user to `/account`. Local data is no longer lost, but on web it is no longer *reachable*, which is most of the user-visible complaint. The fix is cheap because §1c-A already threads the cause: treat `AuthSignOutCause.sessionExpired` as signed-in-offline (`lastKnownUid` survives it, so the library renders), and eject only on `userInitiated`. Guard against the obvious trap: a genuinely signed-out user must still hit the wall, and cloud reads stay RLS-gated regardless.

**KNOWN LIMITATION, deliberately deferred — the row guard's L4 backstop is not reachable in production.** `library_providers.dart` wires both `currentUid` and `hasKnownSession` from the *same* source (`effectiveUidProvider`), so `currentUid() == null` ⟺ `hasKnownSession() == false`, and `_GuardOutcome.identityUnknown` can only be reached by injecting the impossible pair `currentUid: () => null, hasKnownSession: () => true` — which is exactly what all 7 of its new tests do. So:

- **`8e47102` and `b1e3924` overstate their commit messages.** They do not deliver the L4 backstop they claim. L4 is closed by the read door (`lastKnownUid`), not by these guards. Stated here rather than quietly left standing.
- `unexpectedZeroRows` **is** live and genuinely useful (races, ownership changed mid-statement). Only `identityUnknown` is dead.
- **Residual hole:** if `hydrate()` fails on a corrupt local DB *and* no session is restorable, the classifier returns `notOwned` and the write is still a silent no-op — the original L4 symptom, uncaught.
- Fixing it properly needs a design decision: `hasKnownSession` must come from a source *independent* of the effective uid — e.g. "a `lastKnownUid` row exists but could not be read" (corrupt) versus "no row was ever written" (never signed in). Note `hasKnownLocalSessionProvider` is also consumed by the router, so its meaning cannot be changed unilaterally. Deferred until after the sync phases; it is a backstop for an already-closed path.

**⚠ Intermittent test to watch:** during §1f Phase 1, `test/features/library/application/library_providers_test.dart` **A1 flaked once under full-suite parallelism** (`+1685 -1`) and was green in isolation and on every subsequent full run. It sits squarely in §1c-A's uid-door area, so it may be a genuine race between `effectiveUidProvider` and Riverpod v3's scheduler rather than harness noise. It has not reproduced — but if it recurs, treat it as a real defect in the uid door, not a flaky test to retry. Do not paper over it with a retry annotation.

**Minor, recorded:** `settings_store_test.dart`'s "not a synced table" test asserts a hardcoded list and never reads `syncTableNames`, so it would pass if the table *were* synced (real protection is `schema_parity_test.dart`, verified clean). `main_boot_last_known_uid_test.dart` test 1 would pass against a gutted `main.dart` — only test 2's source-order scan pins the boot wiring. And `dafcd43` added an early `return` on `!context.mounted` *before* invoking the mutation, so a write is silently skipped if the context unmounts as the dialog closes — practically unreachable, but it adds a silent skip to the very path meant to eliminate them.
...
3. **Do existing installs already hold `climbtopo` in IndexedDB rather than OPFS?** ~~If so they are pinned there until §1a's `moveExistingIndexedDbToOpfs` ships.~~ **(Corrected 2026-08-01: the flag shipped and was reverted (`09cf076`) — drift 2.34.2's migration can silently destroy the library while reporting a durable verdict. It will not ship again; those installs stay on IndexedDB permanently, which is durable and fine. Full analysis: `docs/superpowers/plans/fragments/1a-storage-interlock.md` §Risks.)**
