...
**CRITICAL coordinate contract:** routes are stored normalized to the **full photo**, and Dart does `Homography.fromQuad(fullPhotoCorners, engineCorners)`. Therefore **every engine must reproject the FULL reference photo's 4 corners** `(0,0),(w,0),(w,h),(0,h)` into view space — never a rock crop. `rockQuadPercent` is used only to *restrict feature detection* (OpenCV mask), never to change which corners are projected.

**Details:**
- `VisionRegistrationEngine` holds an `ArVisionPipeline`, the reference `CGImage`+`refSize`. Vision registers the **whole** reference image (it can't easily mask); `rockQuadPercent` is ignored for Vision v1 (note it). Add a `loadReference(cgImage:refSize:)` overload to `ArVisionPipeline` (set `referenceCGImage`+`referenceSize` directly — the existing one takes a path). `process`: `var out: ArAlignmentResult?; pipeline.processLiveFrame(pixelBuffer) { out = $0 }` (it runs synchronously); `guard let r = out, r.frameWidth > 0 else { return nil }`; the `r.homography` is row-major ref-pixel→live-pixel; reproject the **full reference's 4 corners** through it into live-pixel space; map live-pixel → view-space via the aspect-fill letterbox transform below; build `EngineAlignment`. Instantiate the pipeline with `frameStride: 1`. Confidence: start from the pipeline's finite+non-degenerate check, then grade by corner-quad sanity — convex + area ratio within [0.02, 4.0] of the reference footprint ⇒ up to 0.85, else scale down; non-finite/concave ⇒ 0 and `tracking:false`.
...
@end
```
`.mm`: convert reference CGImage → BGRA `cv::Mat` → gray (**full image, NOT cropped**). Build an ORB **detection mask** from `rockQuad`: a `cv::Mat` of the full gray size, `cv::fillConvexPoly` the rock quad (denormalized to pixels) white on black; empty `rockQuad` ⇒ no mask (detect everywhere). `cv::ORB::create(1500)` detect+compute **on the full gray image with the mask** once — keypoints stay in **full-image pixel coords**; store keypoints/descriptors. `matchPixelBuffer`: lock buffer, wrap BGRA plane → gray Mat, ORB detect+compute (no mask on the live frame), `cv::BFMatcher(cv::NORM_HAMMING)` knnMatch k=2 + Lowe ratio 0.75, require ≥12 good matches, `cv::findHomography(refPts, livePts, cv::RANSAC, 5.0, mask)`, require inliers ≥ 12 and inlierRatio ≥ 0.25, reproject the **FULL reference's 4 corners** `(0,0),(w,0),(w,h),(0,h)` (full-reference-pixel coords) via the homography, return the 8 live-pixel corners + inlierRatio; nil otherwise. Guard empty descriptors.
