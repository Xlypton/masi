...
| flutter_map | 8.3.1 | OK | Works on web; tile fetch is plain http (CORS on tile host). **Shipped, superseding the plan below:** the basemap moved to VECTOR (`flutter_map_vector_tiles` + CARTO Positron) with its own IndexedDB cache (`lib/core/map/vector_tile_cache.dart`); there is no `TileLayer` in `lib` any more — see `CLAUDE.md`'s "Web offline stack". *As originally planned:* on-disk tile cache (`BuiltInMapCachingProvider`) needs `DisabledMapCachingProvider` on web (already used in this repo's tests). |
...
- **Maps:** `flutter_map` + a VECTOR CARTO Positron basemap (`flutter_map_vector_tiles`, via the shared `BasemapLayer`). Works on web; watch style/tile-host CORS. **Shipped, superseding "swap on-disk tile cache → `DisabledMapCachingProvider` on web" as originally planned here:** an IndexedDB-backed cache for both the tiles and the style's own bytes (`lib/core/map/vector_tile_cache.dart`) — see `CLAUDE.md`'s "Web offline stack". Map marker thumbnails hit the §4 photo pipeline.
